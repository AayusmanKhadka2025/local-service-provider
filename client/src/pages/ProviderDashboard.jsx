...
const ProviderDashboard = () => {
  ...
  const statusLabels = {
    pending: "Pending",
    confirmed: "Confirmed - Awaiting Completion",
    completed: "Completed",
    ...
    const loadProviderData = () => {
      ...
      try {
        ...
        setProvider({
          ...
          role: "Service Provider",
          avatar: providerData.profileImage || `https://ui-avatars.com/api/?name=${encodeURIComponent(providerData.firstName || 'Provider')}&background=3b82f6&color=fff&size=100`,
  ...
  const handleAccept = (bookingId) => updateBookingStatus(bookingId, 'confirmed', 'Booking Accepted Successfully! ✅');
  const handleReject = (bookingId) => updateBookingStatus(bookingId, 'rejected', 'Booking Rejected ❌');
  const markAsCompleted = (bookingId) => updateBookingStatus(bookingId, 'completed', 'Work Completed Successfully! 🎉');
  ...
  const getUserImage = (name, email, avatar) => {
    ...
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName)}&background=3b82f6&color=fff&size=100&bold=true`;
  };
  ...
  const totalBookings = bookings.pending.length + bookings.confirmed.length + 
                        bookings.completed.length + bookings.rejected.length + 
  ...
  const stats = [
    ...
    { label: "Rejected/Cancelled", value: rejectedCount + cancelledCount, icon: XCircle, color: "red" },
  ...
  const renderBookingSection = (title, bookingsList, type, onAccept, onReject, onComplete) => {
        ...
        {isEmpty ? (
          <div className="bg-white rounded-xl p-8 text-center border border-gray-100">
            ...
            <p className="text-sm text-gray-500">
              ...
              {type === 'confirmed' && "Accepted bookings waiting to be completed will appear here."}
        ...
        return (
          <>
            ...
            {renderBookingSection(
              ...
              [...bookings.completed, ...bookings.rejected, ...bookings.cancelled],
              "history",
              null,
              null,
              null
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-6">
                <div className="relative">
                  <img src={provider.avatar} className="w-24 h-24 rounded-full border-4 border-blue-100 object-cover" alt={provider.fullName} />
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1 mt-1">{renderStars(provider.rating)}</div>
                </div>
              </div>
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <ProfileField label="Hourly Rate" value={`$${provider.hourlyRate}/hr`} />
                ...
                <div className="p-4 bg-gray-50 rounded-xl">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {provider.serviceTags.map((tag, index) => (
                      <span key={index} className="px-2 py-1 bg-blue-100 text-blue-700 rounded-lg text-xs">{tag}</span>
                    ))}
                  </div>
                </div>
              )}
    ...
    <div className="flex h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <aside className="hidden md:flex w-72 bg-white/90 backdrop-blur-sm flex-col p-6 fixed left-0 top-0 h-screen border-r border-gray-200 shadow-lg">
        ...
        <div className="flex items-center gap-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl mb-6 border border-blue-100">
          <img src={provider.avatar} className="w-12 h-12 rounded-full border-2 border-white shadow-md object-cover" alt={provider.fullName} />
...
const BookingCard = ({ booking, type, onAccept, onReject, onComplete, onUserClick, getUserImage, formatDate, formatPrice, renderStars, statusLabels, statusColors }) => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            <button onClick={onUserClick} className="w-12 h-12 rounded-full overflow-hidden border-2 border-blue-100 hover:scale-105 transition cursor-pointer">
        ...
        <div className="text-right min-w-[180px]">
          ...
          <p className="text-sm font-semibold text-gray-700 mt-2">{formatPrice(booking.totalAmount)}</p>
          
          {type === 'pending' && (
            <div className="flex gap-2 mt-2">
              <button onClick={onReject} className="px-3 py-1.5 bg-red-50 text-red-600 rounded-lg text-xs font-medium hover:bg-red-100 transition">Reject</button>
              <button onClick={onAccept} className="px-3 py-1.5 bg-green-50 text-green-600 rounded-lg text-xs font-medium hover:bg-green-100 transition">Accept</button>
            </div>
          )}
          
          {type === 'confirmed' && (
            <button onClick={onComplete} className="mt-2 px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700 transition flex items-center gap-1 mx-auto">
...
// Profile Field Component (same as before)
...
// User Modal Component (same as before)
const UserModal = ({ user, onClose, getUserImage, formatDate, formatPrice, renderStars, statusLabels, statusColors }) => {
