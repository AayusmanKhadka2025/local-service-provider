...
const ProviderDashboard = () => {
  ...
  useEffect(() => {
    const loadNotificationsFromStorage = () => {
      const storedNotifications = localStorage.getItem("provider_notifications");
  ...
  const saveNotificationsToStorage = (updatedNotifications) => {
    localStorage.setItem("provider_notifications", JSON.stringify(updatedNotifications));
  };
      ...
      try {
        ...
        if (response.data.success) {
          ...
          bookingsData.pending.forEach((booking) => {
            ...
            const alreadyExists = existingNotifications.some(n => n.id === generateNotificationId(booking._id, "new_booking"));
          ...
          bookingsData.cancelled.forEach((booking) => {
            // Check if notification already exists
            const existingNotifications = JSON.parse(localStorage.getItem("provider_notifications") || "[]");
            const alreadyExists = existingNotifications.some(n => n.id === generateNotificationId(booking._id, "booking_cancelled"));
  ...
  const stats = [
    { label: "Total Bookings", value: totalBookings, icon: Briefcase, color: "blue" },
    { label: "Pending Requests", value: pendingCount, icon: Clock, color: "orange" },
    { label: "In Progress", value: inProgressCount, icon: Play, color: "purple" },
    { label: "Completed", value: completedCount, icon: CheckCircle, color: "green" },
    { label: "Total Rating", value: provider.rating.toFixed(1), icon: Star, color: "yellow", suffix: "⭐" },
  ];
  ...
  const renderBookingSection = (title, bookingsList, type, onStart, onComplete) => {
    ...
    return (
      <section className="mb-10" id={type === "pending" ? "pending-bookings" : undefined}>
          ...
          {showViewAll && (
            ...
            >
              {isExpanded ? (
                ...
                <>View All ({bookingsList.length}) <ChevronDown className="w-4 h-4" /></>
              )}
            </button>
          )}
        </div>
        ...
        {isEmpty ? (
          <div className="bg-white rounded-xl p-8 text-center border border-gray-100">
            ...
            <h3 className="text-md font-medium text-gray-700 mb-1">No {title.toLowerCase()}</h3>
            <p className="text-sm text-gray-500">
              {type === "pending" && "When customers book your services, they will appear here."}
              {type === "confirmed" && "Accepted bookings waiting to be started will appear here."}
              {type === "in_progress" && "Services that have been started will appear here."}
              {type === "history" && "Completed, rejected, or cancelled bookings will appear here."}
            </p>
          </div>
        ) : (
  ...
  const renderContent = () => {
    switch (activeTab) {
      case "dashboard":
        return (
          <>
              ...
              {stats.map((stat) => {
                ...
                return (
                  <div key={stat.label} className="bg-white rounded-2xl p-4 md:p-6 shadow-sm border border-gray-100">
                    ...
                    {stat.label === "Total Rating" && <div className="mt-2">{renderStars(provider.rating)}</div>}
                  </div>
                );
              })}
            </div>
            ...
            {/* Booking Sections */}
            {renderBookingSection("Recent Booking Requests", bookings.pending, "pending")}
            {renderBookingSection("Accepted Bookings (Ready to Start)", bookings.confirmed, "confirmed", startService)}
            {renderBookingSection("Services In Progress", bookings.in_progress, "in_progress", null, completeService)}
            {renderBookingSection("Recent Booking History", [...bookings.completed, ...bookings.rejected, ...bookings.cancelled], "history")}
          </>
        );
      ...
      case "earnings":
        return (
          <div className="bg-white rounded-2xl shadow-lg p-6 md:p-8">
            <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-6">Earnings Overview</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 md:p-6">
                <p className="text-sm text-gray-600">Total Earnings</p>
                <p className="text-2xl md:text-3xl font-bold text-gray-800 mt-2">Rs. 4,250</p>
              </div>
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4 md:p-6">
                <p className="text-sm text-gray-600">This Month</p>
                <p className="text-2xl md:text-3xl font-bold text-gray-800 mt-2">Rs. 1,280</p>
              </div>
              <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-4 md:p-6">
                ...
                <p className="text-2xl md:text-3xl font-bold text-gray-800 mt-2">Rs. 145</p>
                <p className="text-xs text-gray-500 mt-2">Based on {bookings.completed.length} completed jobs</p>
              </div>
            </div>
          </div>
        );
      ...
      case "profile":
        return (
          <div className="bg-white rounded-2xl shadow-lg p-6 md:p-8">
            <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-6">Profile Settings</h2>
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <ProfileField label="Service Category" value={provider.category} />
                <ProfileField label="Experience" value={provider.experience} />
                <ProfileField label="Hourly Rate" value={`Rs. ${provider.hourlyRate}/hr`} />
                <ProfileField label="Service Area" value={provider.serviceArea} />
                <ProfileField label="Address" value={`${provider.address}, ${provider.city}`} />
                <ProfileField label="Available Days" value={provider.availableDays?.join(", ")} />
              </div>
      ...
      <aside className="hidden md:flex w-72 bg-white/90 backdrop-blur-sm flex-col p-6 fixed left-0 top-0 h-screen border-r border-gray-200 shadow-lg overflow-y-auto">
        ...
        <div className="flex items-center gap-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl mb-6 border border-blue-100">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-semibold text-gray-800 truncate">{provider.fullName}</p>
                  ...
                  <div className="max-h-96 overflow-y-auto">
                    {notifications.length === 0 ? (
                      <div className="p-8 text-center">
                        ...
                        <p className="text-xs text-gray-400 mt-1">New booking requests and cancellations will appear here</p>
                      </div>
                    ) : (
...
const BookingCard = ({ booking, type, onAccept, onReject, onStart, onComplete, onUserClick, getUserImage, formatDate, formatTime, formatPrice, renderStars, statusLabels, statusColors }) => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-2">
            <button onClick={onUserClick} className="w-10 h-10 md:w-12 md:h-12 rounded-full overflow-hidden border-2 border-blue-100 hover:scale-105 transition cursor-pointer">
              <img src={getUserImage(booking.user.name, booking.user.email, booking.user.avatar)} className="w-full h-full object-cover" alt={booking.user.name} />
            </button>
            <div>
              <button onClick={onUserClick} className="font-semibold text-gray-800 hover:text-blue-600 transition text-left">
                {booking.user.name}
              </button>
          ...
          <div className="space-y-1 ml-12 md:ml-15">
            ...
            {(booking.status === "in_progress" || booking.status === "completed") && (
              <div className="mt-2 pt-2 border-t border-gray-100">
                {booking.startTime && <p className="text-xs text-gray-600"><span className="font-medium">Started:</span> {formatTime(booking.startTime)}</p>}
                {booking.endTime && <p className="text-xs text-gray-600"><span className="font-medium">Completed:</span> {formatTime(booking.endTime)}</p>}
                {booking.duration > 0 && <p className="text-xs text-gray-600"><span className="font-medium">Actual Duration:</span> {booking.duration.toFixed(2)} hours</p>}
                {booking.hoursCharged > 0 && <p className="text-xs text-gray-600"><span className="font-medium">Hours Billed:</span> {booking.hoursCharged} hour(s)</p>}
                {booking.calculatedAmount > 0 && <p className="text-xs font-semibold text-green-600"><span className="font-medium">Total:</span> {formatPrice(booking.calculatedAmount)}</p>}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="text-right min-w-[160px]">
          ...
          {type === "pending" && (
            <div className="flex gap-2 mt-2">
              <button onClick={onReject} className="px-2 py-1 md:px-3 md:py-1.5 bg-red-50 text-red-600 rounded-lg text-xs font-medium hover:bg-red-100 transition">
                Reject
              </button>
              <button onClick={onAccept} className="px-2 py-1 md:px-3 md:py-1.5 bg-green-50 text-green-600 rounded-lg text-xs font-medium hover:bg-green-100 transition">
                Accept
              </button>
            </div>
          )}
          ...
          {type === "confirmed" && (
            <button onClick={onStart} className="mt-2 px-3 py-1.5 md:px-4 md:py-2 bg-green-600 text-white rounded-lg text-xs md:text-sm font-medium hover:bg-green-700 transition flex items-center gap-1 mx-auto">
          ...
          {type === "in_progress" && (
            <button onClick={onComplete} className="mt-2 px-3 py-1.5 md:px-4 md:py-2 bg-blue-600 text-white rounded-lg text-xs md:text-sm font-medium hover:bg-blue-700 transition flex items-center gap-1 mx-auto">
              <CheckCircle className="w-3 h-3 md:w-4 md:h-4" />
              Complete Service
            </button>
          )}

          {type === "history" && booking.status === "completed" && booking.rating && (
            <div className="mt-2">
...
const ProfileField = ({ label, value }) => (
  <div className="p-3 md:p-4 bg-gray-50 rounded-xl">
    ...
    <p className="font-medium text-sm md:text-base">{value || "Not specified"}</p>
  </div>
);

// User Modal Component
const UserModal = ({ user, onClose, getUserImage, formatDate, formatTime, formatPrice, renderStars, statusLabels, statusColors }) => {
  const statusColor = statusColors[user.bookingDetails?.status] || statusColors.pending;
  const statusLabel = statusLabels[user.bookingDetails?.status] || user.bookingDetails?.status;
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden animate-scaleIn">
        ...
        <div className="p-6 max-h-[70vh] overflow-y-auto">
          <div className="flex items-center gap-4 mb-6">
            <img src={getUserImage(user.name, user.email, user.avatar)} className="w-20 h-20 rounded-full border-4 border-blue-100 object-cover" alt={user.name} />
            <div>
              ...
              {user.emergencyContact && (
                <div className="flex items-center gap-2 text-sm text-orange-600 mt-1">
                  <Phone className="w-4 h-4" />
                  <span className="font-medium">Emergency: {user.emergencyContact}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="border-t border-gray-100 pt-4 space-y-3">
            ...
            <div className="space-y-2">
              <p className="text-sm"><span className="font-medium text-gray-700">Service:</span> {user.bookingDetails?.service}</p>
              <p className="text-sm"><span className="font-medium text-gray-700">Address:</span> {user.bookingDetails?.user?.address}</p>
              <p className="text-sm"><span className="font-medium text-gray-700">Date & Time:</span> {formatDate(user.bookingDetails?.date)} at {user.bookingDetails?.time}</p>
              {user.bookingDetails?.startTime && <p className="text-sm"><span className="font-medium text-gray-700">Started:</span> {formatTime(user.bookingDetails?.startTime)}</p>}
              {user.bookingDetails?.endTime && <p className="text-sm"><span className="font-medium text-gray-700">Completed:</span> {formatTime(user.bookingDetails?.endTime)}</p>}
              {user.bookingDetails?.duration > 0 && <p className="text-sm"><span className="font-medium text-gray-700">Duration:</span> {user.bookingDetails?.duration} hour(s)</p>}
              {user.bookingDetails?.calculatedAmount > 0 && <p className="text-sm"><span className="font-medium text-gray-700">Total Amount:</span> {formatPrice(user.bookingDetails?.calculatedAmount)}</p>}
              ...
              <p className="text-sm"><span className="font-medium text-gray-700">Description:</span> {user.bookingDetails?.instructions || "No additional instructions"}</p>
            </div>
          </div>

          {user.bookingDetails?.status === "completed" && user.bookingDetails?.rating && (
            <div className="border-t border-gray-100 mt-4 pt-4">
              <h5 className="font-semibold text-gray-800 flex items-center gap-2 mb-2">
                <MessageSquare className="w-4 h-4 text-blue-600" />
                Customer Review
              </h5>
              <div className="mb-2">{renderStars(user.bookingDetails.rating)}</div>
              <p className="text-sm text-gray-600 italic">"{user.bookingDetails.review}"</p>
            </div>
          )}
        </div>
      </div>
