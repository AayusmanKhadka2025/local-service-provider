...
const UserChat = ({ booking, provider, onClose }) => {
  ...
  const isBlocked = isBlockedByUser || isBlockedByProvider;
    ...
    const initializeChat = async () => {
      ...
      try {
        ...
        if (response.data.success && response.data.chat) {
          ...
          // Explicitly join room in case socket connected before chat was resolved
          if (socket) {
  ...
  useEffect(() => {
    if (chat?._id && !loading && messages.length > 0 && !isBlocked) {
      const markMessagesAsRead = async () => {
        try {
          await axios.post(
            `http://localhost:5050/api/chat/mark-read/${chat._id}`,
            {},
            { headers: { Authorization: `Bearer ${token}` } },
          );

          if (socket) {
            socket.emit("mark_read", { chatId: chat._id, userId: user._id });
          }
        } catch (error) {
  ...
  }, [chat?._id, loading, messages.length, user._id, socket, token, isBlocked]);
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (message) => {
      const currentChatId = chatRef.current?._id?.toString();
      const messageChatId = message.chatId?.toString();

      if (!currentChatId || !messageChatId) return;
    ...
    const handleUserTyping = (data) => {
      if (data.userId === provider._id && !isBlockedByUser) {
  ...
  }, [socket, provider._id, user._id, isBlockedByUser]);
  ...
  const handleBlockChat = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5050/api/chat/block/${chat._id}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } },
      );

      if (response.data.success) {
        setChat((prev) => ({ ...prev, isBlocked: true, blockedBy: "user" }));
  ...
  // Unblock chat with real-time sync
  const handleUnblockChat = async () => {
    setActionLoading(true);
    try {
      const response = await axios.post(
        `http://localhost:5050/api/chat/unblock/${chat._id}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const handleReportChat = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5050/api/chat/report/${chat._id}`,
  ...
  const handleTyping = useCallback(
    ...
    [isTyping, chat, socket, user._id, isBlocked],
  );
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-2xl shadow-xl overflow-hidden">
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-orange-100 text-sm mt-1">
                    Please tell us why you're reporting this conversation
                  </p>
                </div>
                <button
            ...
            <div className="p-6">
              ...
              {reportReason === "other" && (
                <div className="mb-4">
                  ...
                  <textarea
                    ...
                    placeholder="Describe why you're reporting this chat..."
                    className="w-full border border-gray-200 rounded-xl px-4 py-2 text-sm focus:ring-2 focus:ring-orange-500 focus:border-transparent outline-none resize-none"
                  />
                </div>
              )}
              <div className="mb-4">
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-3 bg-orange-600 text-white rounded-xl font-medium hover:bg-orange-700 transition flex items-center justify-center gap-2 disabled:opacity-50"
                >
                  {actionLoading ? (
            ...
            <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-gray-200 z-10 overflow-hidden">
              {isBlockedByUser ? (
                <button
                  ...
                  className="w-full px-4 py-2 text-left text-sm text-green-600 hover:bg-green-50 flex items-center gap-2 transition"
                >
                ...
                <button
                  ...
                  className="w-full px-4 py-2 text-left text-sm text-orange-600 hover:bg-orange-50 flex items-center gap-2 transition"
                >
        ...
        <div className="bg-orange-50 border-b border-orange-200 p-3 text-center">
          <div className="flex items-center justify-center gap-2">
            ...
            <span className="text-sm text-orange-700">
              You have blocked this conversation. You cannot send new messages.
            </span>
            <button
              onClick={() => setShowUnblockConfirm(true)}
        ...
        <div className="bg-red-50 border-b border-red-200 p-3 text-center">
          <div className="flex items-center justify-center gap-2">
            ...
            <span className="text-sm text-red-700">
              This conversation has been blocked. You cannot send new messages.
            </span>
          </div>
        </div>
      )}
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-yellow-700 mt-1">
                You will not be able to send or receive messages from this
                person. Your existing messages will remain visible.
              </p>
      ...
      {showUnblockConfirm && isBlockedByUser && (
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
        ...
        {otherUserTyping && !isBlockedByUser && (
      ...
      <div className="p-4 border-t border-gray-200 bg-white">
        <div
          className={`flex items-center gap-2 ${isBlocked ? "opacity-50" : ""}`}
        >
          <button
          ...
          <input
            ...
            placeholder={
              isBlocked
                ? "Conversation blocked - cannot send messages"
                : "Type a message..."
            }
